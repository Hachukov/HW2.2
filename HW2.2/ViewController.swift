...
final class ViewController: UIViewController{
    ...
    @IBAction func doneButton() {
        delegate.shangeCollor(colorView.backgroundColor!)
        dismiss(animated: true)
        
    }

    func changeCollor(_ collor: UIColor) {

        self.colorView.backgroundColor = collor
        
    }
    
    func convertCollor() {
        let collor =  changeCollor
        let red = Float((collor?.rgba.red)!)
        let green = Float((collor?.rgba.green)!)
        let blue = Float((collor?.rgba.blue)!)
        ...
        colorView.backgroundColor = changeCollor
